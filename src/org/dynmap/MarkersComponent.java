...
/**
 * Markers component - ties in the component system, both on the server and
 * client
 */
public class MarkersComponent extends ClientComponent {
    ...
    public MarkersComponent(final DynmapCore core, ConfigurationNode configuration) {
        ...
        if (showSpawn) {
            ...
            spawnicon = api.getMarkerIcon(ico);
            /* Load it */
            if (spawnicon == null) {
       ...
       /* if (configuration.getBoolean("showofflineplayers", false)) {
            ...
            maxofflineage = 60000L * configuration.getInteger("maxofflinetime", 30);
            /* 30 minutes */
 /* Now, see if existing offline markers - check for last login on their users *
            if (maxofflineage > 0) {
                Set<Marker> prev_m = offlineset.getMarkers();
                for (Marker m : prev_m) {
                    Player p = Bukkit.getPlayerExact(m.getMarkerID());
                    if (p != null) {
                        long ageout = p.getLastLogin() + maxofflineage;
       ...
       /* if (configuration.getBoolean("showspawnbeds", false)) {
            ...
            core.listenerManager.addListener(EventType.PLAYER_BED_LEAVE, new PlayerEventListener() {
                @Override
                public void playerEvent(final Player p) {
                    core.getServer().scheduleServerTask(new Runnable() {
                        public void run() {
                            updatePlayer(p);
                        }
    ...
    /*private void updatePlayer(Player p) {
        DynmapLocation bl = new DynmapLocation(p.getBedSpawnLocation());
        Marker m = spawnbedset.findMarker(p.getName() + "_bed");
        if (bl == null) {
            /* No bed location *
            if (m != null) {
