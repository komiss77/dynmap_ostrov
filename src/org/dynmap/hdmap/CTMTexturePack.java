...
public class CTMTexturePack {
    ...
    private List<String>  biomenames = new ArrayList<>();
            ...
            for (String token : property.split("\\s+")) {
                if (token.equals("")) {
                    ...
                    try {
                        ...
                        DynmapBlockState bs = null;//DynmapBlockState.getStateByLegacyBlockID(id);
                        if (bs == null) {
        ...
        private void getBiomes(Properties p, CTMTexturePack tp) {
            ...
            if (!v.equals("")) {
                ...
                for(String s : v.split("\\s+")) {
                    for(int i = 0; i < tp.biomenames.size(); i++) {
                        if(s.equals(tp.biomenames.get(i))) {//biomenames[i])) {
                            ids.add(i);
                            s = null;
                            break;
                        }
...
//            }
    ...
    private void processFiles(DynmapCore core) {        
        ...
        for (String name : biomenames) {
            l.add(name.toLowerCase().replace(" ", ""));
        }
        biomenames = l;
        //String[] newbiomes = new String[biomenames.length];
        //for(String n : biomenames) {//(int i = 0; i < biomenames.length; i++) {
...
//        }
    ...
    private class Context {
        ...
        Context(OurMapIterator mapiter, DynmapBlockState blk, BlockStep laststep, int textid) {
    ...
    public int mapTexture(OurMapIterator mapiter, DynmapBlockState blk, BlockStep laststep, int textid, HDShaderState ss) {
        int newtext = -1;
