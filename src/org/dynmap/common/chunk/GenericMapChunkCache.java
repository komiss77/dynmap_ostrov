...
public abstract class GenericMapChunkCache extends MapChunkCache {
     ...
     */
    public class OurMapIterator implements MapDataContext//implements MapIterator 
    {
        ...
        public final void initialize(int x0, int y0, int z0) {
            ...
            if ((chunkindex >= snapcnt) || (chunkindex < 0)) {
                snap = getEmpty();
//Ostrov.log_warn("OurMapIterator initialize " + x + "x" + y + "x" + z + " idx=" + chunkindex + " " + bx + "x" + bz + " Empty!");
            } else {
                snap = snaparray[chunkindex];
//Ostrov.log_warn("OurMapIterator initialize " + x + "x" + y + "x" + z + " idx=" + chunkindex + " " + bx + "x" + bz + " snap=" + snap.chunkStatus);
            }
    ...
    /**
     * Read NBT data from loaded chunks - do not needs to be called from
     * server/world
     * <p>
    ...
    /**
     * It loads chunks from the cache or from the world, and if the chunk is not
     * visible, it fills it with stone, ocean or empty chunk
     * <p>
     ...
     * Get cache iterator
     */
    public OurMapIterator getIterator(int x, int y, int z) {
    ...
    /**
     * Add visible area limit - can be called more than once Needs to be set
     * before chunks are loaded Coordinates are block coordinates
     */
    ...
    /**
     * Add hidden area limit - can be called more than once Needs to be set
     * before chunks are loaded Coordinates are block coordinates
     */
