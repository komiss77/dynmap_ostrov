...
public class GenericChunk {
    ...
    public final DynmapBlockState getBlockType(int x, int y, int z) {
        GenericChunkSection gs = getSection(y);
//Ostrov.log_warn("DynmapBlockState getBlockType " + x + "x" + y + "x" + z + " gs=" + gs+" bl="+gs.blocks.getBlock(x, y, z));
                ...
                for (int y = 15; (y >= 0) && (!allzero); y--) {
                    ...
                    for (int x_ = 0; x_ < 16; x_++) {
                        for (int z_ = 0; z_ < 16; z_++) {
                            int idx = (z_ << 4) + x_;
                            int val = sky[idx];
                            DynmapBlockState bs = sect.blocks.getBlock(x_, y, z_);	// Get block
                        ...
                        do {
                            ...
                            for (int x_ = 0; x_ < 16; x_++) {
                                for (int z_ = 0; z_ < 16; z_++) {
                                    ...
                                    if (x_ < 15) {	// If not right edge, check X spread
                                        int right = sky[idx + 1];
                                    ...
                                    if (z_ < 15) {	// If not bottom edge, check Z spread
                                        int down = sky[idx + 16];
